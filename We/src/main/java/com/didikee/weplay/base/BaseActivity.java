...
//    private final int TITLE_HEIGHT=DisplayUtil.dp2px(this,40);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TITLE_ITEM_WIDTH=DisplayUtil.dp2px(this,40);
    }
...
//        left.setImageResource(R.drawable.ic_menu_back);
    ...
    protected void setDefaultLeftViewParams(@Nullable Integer drawableId,@Nullable String text, @Nullable ImageView.ScaleType scaleType){
        FrameLayout mDefaultTitleGroup = (FrameLayout) mBaseContent.getChildAt(0);
        ImageTextView itvLeft= (ImageTextView) mDefaultTitleGroup.findViewById(R.id.LEFT);

        if (drawableId!=null){
            itvLeft.setImageResource(drawableId);
        }

        if (!TextUtils.isEmpty(text)){
            itvLeft.setText(text);
        }

        if (scaleType!=null){
            itvLeft.setScaleType(scaleType);
        }
    }
